...
import type { Habit, Answer, Trait, TraitScores } from "./types";
...
// Attach a stable fake respondent count to each queue entry at shuffle time
interface QueueItem extends Habit {
  respondents: number;
}

function buildQueue(source: Habit[]): QueueItem[] {
  return shuffle(source).map((h) => ({ ...h, respondents: fakeRespondents() }));
}

export default function App() {
  const [view, setView] = useState<View>("landing");
  const [queue, setQueue] = useState<QueueItem[]>(() => buildQueue(HABITS));
  const [queueIndex, setQueueIndex] = useState(0);
  const [userAnswer, setUserAnswer] = useState<Answer | null>(null);
  const [streak, setStreak] = useState(0);

  // Trait scoring — two parallel records: earned points and maximum possible
  ...
  const handleAnswer = (answer: Answer) => {
    ...
    // Every shown habit contributes to the possible maximum
    setTraitMax((prev) => addTraitPoints(prev, current.traits));

    // Only "Me Too" answers earn scored points
    if (answer === "me-too") {
      setTraitScores((prev) => addTraitPoints(prev, current.traits));
    }
  ...
  const resultMessage = userAnswer ? getResultMessage(current.meTooPct, userAnswer) : "";
          ...
          {view === "landing" && (
            ...
            >
              ...
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.95 }}
                onClick={() => setView("question")} data-testid="button-start"
            ...
            >
              ...
              <div className="flex gap-3">
                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.93 }}
                  onClick={() => handleAnswer("not-me")} data-testid="button-not-me"
                ...
                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.93 }}
                  onClick={() => handleAnswer("me-too")} data-testid="button-me-too"
            ...
            >
              <div className="w-full bg-card rounded-[2rem] p-8 shadow-2xl shadow-black/30 flex flex-col relative overflow-hidden">
                ...
                <div className="flex flex-col items-center z-10 space-y-1 mb-4">
                  <div className="flex items-baseline gap-1 text-primary leading-none">
                    <span className="text-[88px] font-extrabold tracking-tighter leading-none"
                      data-testid="text-result-percentage">
                      <AnimatedCounter target={current.meTooPct} />
                    </span>
                ...
                <p className="text-center text-card-foreground/40 text-sm font-medium z-10 mb-5"
                  data-testid="text-respondents">
                  {formatRespondents(current.respondents)} people answered
                </p>

                {/* Profile unlock teaser */}
                ...
                <div className="z-10 space-y-3">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.96 }}
                    onClick={handleNext} data-testid="button-next"
              ...
              {next && !profileUnlocked && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <span className="text-white font-semibold text-sm">
                    {CATEGORY_EMOJI[next.category]} {next.category}
                  </span>
                </motion.div>
              )}
            </motion.div>
            ...
            >
              <div className="w-full bg-card rounded-[2rem] p-8 shadow-2xl shadow-black/30 relative overflow-hidden">
                ...
                <div className="mt-6 pt-5 border-t border-purple-100 z-10 relative">
                  <p className="text-card-foreground/40 text-xs text-center">
                    Based on {streak} habits answered
                  </p>
                </div>
              </div>

              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.96 }}
                onClick={() => setView("question")} data-testid="button-keep-discovering"
