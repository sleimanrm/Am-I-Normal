...
import { getApprovedAsHabits } from "./data/submissions";
import {
  CATEGORY_EMOJI,
  TRAIT_EMOJI,
  TRAIT_DESCRIPTION,
...
function buildQueue(): QueueItem[] {
  const community = getApprovedAsHabits();
  const all = [...HABITS, ...community];
  return shuffle(all).map((h) => ({ ...h, respondents: fakeRespondents() }));
}
...
export default function GameScreen() {
  ...
  // Rebuild queue when returning to landing (picks up newly approved submissions)
  const handleStart = () => {
    setQueue(buildQueue());
    setQueueIndex(0);
    setView("question");
          ...
          {view === "landing" && (
            ...
            >
              ...
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.95 }}
                onClick={handleStart} data-testid="button-start"
      ...
      <AnimatePresence>
        {showFloatingAdd && (
          <motion.button
            ...
            data-testid="button-share-yours"
